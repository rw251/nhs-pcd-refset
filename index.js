...
function getFileNames(dir, startingFromProjectDir) {
  ...
  const definitionFileBrotli = path.join(processedFilesDirFromRoot, "pcd-defs.json.br");
  const refSetFileBrotli = path.join(processedFilesDirFromRoot, "pcd-refSets.json.br");
  return {
    rawFilesDir,
    definitionJsonFile,
    refSetJsonFile,
...
async function loadDataIntoMemory({ dirName }) {
  const { processedFilesDirFromRoot, rawFilesDir, definitionJsonFile, refSetJsonFile } =
  ...
  if (unknownCodes.length > 0) {
    console.log(`> There are ${unknownCodes.length} codes without a definition.term`);
...
function compressJson(dir) {
  const { definitionJsonFile, refSetJsonFile, definitionFileBrotli, refSetFileBrotli } =
    getFileNames(dir);
